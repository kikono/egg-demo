...
module.exports = app => {
  ...
  loadModel(app,config.domainPath);
...
function loadModel(app, domainPath) {
  domainPath = (!domainPath)? 'app/model':domainPath;
  const dir = path.join(app.config.baseDir, domainPath);
